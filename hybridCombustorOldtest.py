...
def CombustorPZ(Params, gas):
    ...
    for i in range(PZ_n_reactor):
        ...
        # final reactor state (from Cantera reactor)
        T_out = reactor.reactor.thermo.T
        Y_out = reactor.reactor.thermo.Y  # or Y_out = reactor.reactor.thermo.Y

        # Reproject the outlet to P0 exactly like the original path does
        gas_out = ct.Solution(Params.RM, transport_model='None')
        gas_out.TPY = T_out, P0, Y_out

        """ Params.PZ_states.append(gas_out)

        rho_out = float(gas_out.density)  # at (T_out, P0, Y_out)
        tres = rho_out * Params.PZ_V_lst[i] / Params.PZ_mdot_in_lst[i]
        Params.PZ_tres_lst.append(tres)

        q = ct.Quantity(gas_out, constant='HP')
        q.mass = Params.PZ_mdot_in_lst[i]
        streamQuantityList.append(q)
        Params.PZ_streams.append(q)  # for diagnostics """

        # 1) Snapshot for diagnostics (never used in mixing)
        state_snap = ct.Solution(Params.RM, transport_model='None')
        state_snap.TPX = gas_out.T, gas_out.P, gas_out.X
        Params.PZ_states.append(state_snap)

        # 2) Residence time at P0 from the snapshot
        rho_out = float(state_snap.density)
        tres = rho_out * Params.PZ_V_lst[i] / Params.PZ_mdot_in_lst[i]
        Params.PZ_tres_lst.append(tres)

        # 3) Separate object for the mixer stream (do NOT share with PZ_states)
        gas_mix = ct.Solution(Params.RM, transport_model='None')
        # keep the same enthalpy @ P0 for HP-constant mixing
        gas_mix.HPX = gas_out.enthalpy_mass, P0, gas_out.X

        q = ct.Quantity(gas_mix, constant='HP')
        q.mass = Params.PZ_mdot_in_lst[i]
        streamQuantityList.append(q)
        ...
        """ gas_out = ct.Solution(Params.RM, transport_model='None')
        gas_out.HPX = reactor.reactor.thermo.enthalpy_mass, P0, reactor.reactor.thermo.X
        ...
        rho = float(gas_out.density)  # gas_out is already at (H,P0,X) so this is ρ(T_out(P0),P0,X_out)
        tres = rho * Params.PZ_V_lst[i] / Params.PZ_mdot_in_lst[i]
        Params.PZ_tres_lst.append(tres) """
