...
class PSR_Plasma:
    def __init__(self, gas_feed, V, mdot_in, T0, P0, EN_func=None, debug=False, gas0=None, mdot_air=None, mdot_fuel=None, L_fuel_inlet=0.0, fuelStr=None, oxidizerStr=None, k=None):
        """
        ...
        self.reactor = ct.IdealGasReactor(self.rgas, energy='on', volume=V)
        ...
        if (mdot_air is not None) and (mdot_fuel is not None) and (fuelStr is not None) and (oxidizerStr is not None):
            ...
            K = 0.0 if (self.k is None) else float(self.k)
            self.leak = ct.Valve(self.reactor, self.outlet)
            # Set coefficient across Cantera versions
            if hasattr(self.leak, "K"):
                self.leak.K = K
            elif hasattr(self.leak, "set_valve_coeff"):
                self.leak.set_valve_coeff(K)
