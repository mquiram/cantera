...
import CombUtils
from hybridReactors import PSR_Plasma
...
def CombustorPZ(Params, gas):
    ...
    for i in range(PZ_n_reactor):
        ...
        gas_feed = ct.Solution(Params.RM, transport_model='None')
        gas_feed.TP = T0, P0
        gas_feed.set_equivalence_ratio(Params.PZ_phi_lst[i], Params.fuelStr, Params.Oxidizer)

        gas0 = ct.Solution(Params.RM, transport_model='None')
        gas0.TPX = T0, P0, Params.Oxidizer

        # Set initial composition for this reactor
        gas0.set_equivalence_ratio(Params.PZ_phi_lst[i], Params.fuelStr, Params.Oxidizer)

        # 1) Fuel vaporization enthalpy correction (match original)
        #    Get pure-fuel enthalpy at (T0,P0)
        gas_tmp = ct.Solution(Params.RM, transport_model='None')
        gas_tmp.TPX = T0, P0, {Params.fuelStr: 1.0}
        h_fuel_T0P0 = gas_tmp.enthalpy_mass

        #    Mixture enthalpy at (T0,P0) with current phi
        h_mix_T0P0  = gas0.enthalpy_mass

        #    Original constants/inputs
        mdot_in_i   = Params.PZ_mdot_in_lst[i]         # total in (air + fuel) for this bin
        mdot_fuel_i = Params.PZ_mdot_fuel_lst[i]
        mdot_air_i  = mdot_in_i - mdot_fuel_i
        L_fuel      = 61.1e3 / 0.17033                 # ≈ 3.586e5 J/kg (same as original)
        h_fuel_std  = Params.fuel_std_enthalpy_mass     # from your setup

        #    Compute post-vaporization mixture enthalpy, then set (H,P)
        h_after = (mdot_in_i*h_mix_T0P0
                - mdot_fuel_i*(L_fuel + (h_fuel_T0P0 - h_fuel_std))) / mdot_in_i
        gas0.HP = h_after, P0
        T_after_vap = gas0.T
        print(h_after)
        print(P0)

        # [Optional but matches original IC exactly]
        # 2) Pre-equilibrate at constant (H,P). If it fails, fall back gently.
        gas0.equilibrate('HP')   # default 'equil' solver, like the original
        ...
        reactor = PSR_Plasma(gas_feed,
                      ...
                      gas0 = gas0,
                      mdot_air=mdot_air_i,
                      mdot_fuel=mdot_fuel_i,
                      L_fuel_inlet=61.1e3/0.17033,          # same number you used for NC12 (set 0.0 for CH4)
        ...
        gas0.HPX = reactor.reactor.thermo.enthalpy_mass, P0, reactor.reactor.thermo.X
        streamQuantity = ct.Quantity(gas0, constant='HP')
